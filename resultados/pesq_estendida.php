<?php
    ...
    if ($GLOBALS['tipo_previsao'] == "prev_estendida") {
        ...
        if ($result[5]['condicao'] == "Predomínio de Sol") {
            $img_previsao06 = './imagens/sol.PNG';
        }

        if ($result[5]['condicao'] == "Nublado") {
            $img_previsao06 = './imagens/nublado.PNG';
        }

        if ($result[5]['condicao'] == "Tempestade") {
            $img_previsao06 = './imagens/tempestade.PNG';
        }

        if ($result[5]['condicao'] == "Chuvas Isoladas") {
            $img_previsao06 = './imagens/chuva_isolada.PNG';
        }

        if ($result[5]['condicao'] == "Parcialmente Nublado") {
            $img_previsao06 = './imagens/parcialmente_nublado.PNG';
        }

        foreach ($grupo01 as $value) {
            if ($value == $result[5]['condicao']) {
                $img_previsao06 = './imagens/chuvoso.PNG';
            }
        }
        foreach ($grupo02 as $value) {
            if ($value == $result[5]['condicao']) {
                $img_previsao06 = './imagens/chuva_manha_tarde.PNG';
            }
        }
        foreach ($grupo03 as $value) {
            if ($value == $result[5]['condicao']) {
                $img_previsao06 = './imagens/chuva_noite.PNG';
            }
        }

        /* Imagem 07 */

        if ($result[6]['condicao'] == "Predomínio de Sol") {
            $img_previsao07 = './imagens/sol.PNG';
        }

        if ($result[6]['condicao'] == "Nublado") {
            $img_previsao07 = './imagens/nublado.PNG';
        }

        if ($result[6]['condicao'] == "Tempestade") {
            $img_previsao07 = './imagens/tempestade.PNG';
        }

        if ($result[6]['condicao'] == "Chuvas Isoladas") {
            $img_previsao07 = './imagens/chuva_isolada.PNG';
        }

        if ($result[6]['condicao'] == "Parcialmente Nublado") {
            $img_previsao07 = './imagens/parcialmente_nublado.PNG';
        }

        foreach ($grupo01 as $value) {
            if ($value == $result[6]['condicao']) {
                $img_previsao07 = './imagens/chuvoso.PNG';
            }
        }
        foreach ($grupo02 as $value) {
            if ($value == $result[6]['condicao']) {
                $img_previsao07 = './imagens/chuva_manha_tarde.PNG';
            }
        }
        foreach ($grupo03 as $value) {
            if ($value == $result[6]['condicao']) {
                $img_previsao07 = './imagens/chuva_noite.PNG';
            }
        }
        ...
        '<div class="row row_result">'.
            ...
            '<div class="col-md-6 result_previsoes">'.
                ...
                '<div class="row">'.
                '<div class="col-md-8">'.
                '<p><span>Previsão para:</span> '.$result[5]['data'].'</p>'.
                '<p><span>Condição:</span> '.$result[5]['condicao'].'</p>'.
                '<p><span>Mínima:</span> '.$result[5]['minima'].'°</p>'.
                '<p><span>Máxima:</span> '.$result[5]['maxima'].'°</p>'.
                '<p><span>IUV:</span> '.$result[5]['iuv'].'</p>'.
                '</div>'.
                '<div class="col-md-4">'.
                '<img src='.$img_previsao06.' class="img-fluid">'.
                '</div>'.
                '</div>'.
            '</div>'.
            '<div class="col-md-3"></div>'.
        ...
        '<div class="row row_result">'.
            ...
            '<div class="col-md-6 result_previsoes">'.
                ...
                '<div class="row">'.
                '<div class="col-md-8">'.
                '<p><span>Previsão para:</span> '.$result[6]['data'].'</p>'.
                '<p><span>Condição:</span> '.$result[6]['condicao'].'</p>'.
                '<p><span>Mínima:</span> '.$result[6]['minima'].'°</p>'.
                '<p><span>Máxima:</span> '.$result[6]['maxima'].'°</p>'.
                '<p><span>IUV:</span> '.$result[6]['iuv'].'</p>'.
                '</div>'.
                '<div class="col-md-4">'.
                '<img src='.$img_previsao07.' class="img-fluid">'.
                '</div>'.
                '</div>'.
            '</div>'.
            '</div>'.
